  ...
  const fetchApiDocumentation = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.BACKEND_URL}/mockapis/serverpeuser/loggedinuser/all-endpoints`,
            ...
            <div className="p-6 bg-gradient-to-br from-indigo-900/30 to-indigo-900/10 border border-indigo-500/30 rounded-2xl space-y-4 animate-slideUp">
              ...
              <div className="mt-3 p-3 bg-yellow-900/20 border border-yellow-700/30 rounded-lg flex gap-2">
                ...
                <p className="text-xs text-yellow-200/80">
                  <strong>Never expose your x-api-key</strong> in client-side
                  code, repositories, or logs. Only use it server-to-server.
                </p>
              </div>
            </div>
